...
! function(t, a) {
    ...
    var i = function(t, e, a) {
        ...
        if ("object" == typeof e.locale && (
                ...
                "string" == typeof e.locale.dropdownRangeLabel && (this.locale.dropdownRangeLabel = e.locale.dropdownRangeLabel),
        ...
        if (this.singleDatePicker) {
            ...
            !this.timePicker && this.autoApply && this.container.addClass("auto-apply")  // Added on Jan 6
        }
        ...
        this.container.find(".drp-calendar")
            .on("click.daterangepicker", ".prev.available", R.proxy(this.clickPrev, this))
            .on("click.daterangepicker", ".next.available", R.proxy(this.clickNext, this))
            .on("keydown.daterangepicker", ".prev.available", R.proxy(this._keydownPrev, this)) //
            .on("keydown.daterangepicker", ".next.available", R.proxy(this._keydownNext, this)) //
    ...
    return i.prototype =
        ...
        updateCalendars: function() {
            ...
            this.renderCalendar("left"),
                ...
                this.showRangesAsDropdown ? this.container.find(".ranges option").prop("checked", false) : this.container.find(".ranges li").removeClass("active"),
        ...
        renderCalendar: function(t) {
            ...
            var k = "left" == t ? this.minDate : this.startDate,
                b = this.maxDate,
                C = ("left" == t ? this.startDate : this.endDate, this.locale.direction, '<div class="calendar-header">');
            console.log(this.startDate);
            k && !k.isBefore(e.firstDay) || this.linkedCalendars && "left" != t ? C += '<div class="prev"></div>' : C += '<div class="prev available" tabindex="0"><span></span></div>';
            var v = this.locale.monthNames[e[1][1].month()] + e[1][1].format(" YYYY");
            if (this.showDropdowns) {
                console.log(t);
                for (var Y = e[1][1].month(), w = e[1][1].year(), P = b && (console.log('year 2 ' + b.year()), b.year()) || this.maxYear, x = k && k.year() || (console.log('year1 ' + this.minYear), this.minYear), M = w == x, S = w == P, I = '<select class="monthselect form-control form-control-sm">', B = 0; B < 12; B++)(!M || B >= k.month()) && (!S || B <= b.month()) ? I += "<option value='" + B + "'" + (B === Y ? " selected='selected'" : "") + ">" + this.locale.monthNames[B] + "</option>" : I += "<option value='" + B + "'" + (B === Y ? " selected='selected'" : "") + " disabled='disabled'>" + this.locale.monthNames[B] + "</option>";
                I += "</select>";
                for (var A = '<select class="yearselect form-control form-control-sm">', L = x; L <= P; L++) A += '<option value="' + L + '"' + (L === w ? ' selected="selected"' : "") + ">" + L + "</option>";
                v = I + (A += "</select>")
            }
            if (C += '<div class="month">' + v + "</div>",
                b && !b.isAfter(e.lastDay) || this.linkedCalendars && "right" != t && !this.singleDatePicker ? C += '<div class="next"></div>' : C += '<div class="next available" tabindex="0"><span></span></div>',
                C += "</div>",
                C += '<table class="table-condensed' + (this.showWeekNumbers || this.showISOWeekNumbers ? ' week' : '' ) + '">' + '<thead><tr>',
                ...
                if (13 === t.keyCode) {
                    ...
                    if (drp.hasClass('right')) {
                        if (r.isAfter(h.calendar.lastDay) || jNext) {
                            drp.find('.next.available').trigger("click.daterangepicker");
                        ...
                        if (r.isBefore(o.calendar.firstDay) || jPrev) {
                            drp.find('.prev.available').trigger("click.daterangepicker")
                            left.find("tbody td").each(function (t, e) {
                                if (!R(e).hasClass("week")) {
                                    var a = R(e).attr("data-title"),
                                        i = a.substr(1, 1),
            ...
            if (this.showRangesAsDropdown) {
                ...
                t && (
                    this.showCustomRangeLabel ?
                        (this.container.find(".ranges li:last").attr("tabindex", 0),
        ...
        // monthOrYearChanged:function(t){
        //     var a=R(t.target).closest(".drp-calendar").hasClass("left"),
        //         i=a?"left":"right",
